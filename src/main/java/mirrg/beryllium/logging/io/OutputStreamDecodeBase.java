...
public abstract class OutputStreamDecodeBase extends OutputStream
{
	...
	public OutputStreamDecodeBase()
	{
		this(Charset.defaultCharset());
	}

	public OutputStreamDecodeBase(String charset)
	{
		this(Charset.forName(charset));
	}

	public OutputStreamDecodeBase(Charset charset)
	{
	...
	private void accept(char[] string, int off, int len) throws IOException
	{
		for (int i = 0; i < len; i++) {
			if (string[i + off] == '\r') {
	...
	protected abstract void println(String string) throws IOException;
